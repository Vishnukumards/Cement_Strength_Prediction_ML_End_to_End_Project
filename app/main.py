...
async def root():
    return {
        "message": "Concrete Strength Prediction API", 
        "docs": "/docs",
...
# NEW: Additional endpoints for production monitoring
@app.get("/info")
async def deployment_info():
    """Get deployment information"""
    return {
        "service": "Concrete Strength Prediction API",
        "version": "1.0.0",
        "environment": os.getenv("RENDER", "development"),
        "deployment": "render.com" if os.getenv("RENDER") else "local",
        "timestamp": datetime.utcnow().isoformat()
    }
